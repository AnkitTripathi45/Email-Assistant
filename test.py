...
def main():
    ...
    # Add autoscroll JS to the page head
    st.markdown("""
    <script>
    ...
    st.markdown("""
        <div class='app-header'>
            <h2>✉️ Email Assistant</h2>
            <p>Generate Professional Email Responses with Different Tones</p>
        </div>
    """, unsafe_allow_html=True)
    ...
    with output_col:
        ...
        if 'generated_response' in st.session_state:
            ...
            # Add auto-scroll without creating a visible component
            if st.session_state.get('should_scroll', False):
                st.markdown("""
                <iframe src="data:text/html;charset=utf-8,
                    <script>
                        parent.scrollToElement('response-section');
                    </script>"
                    style="display: none;">
                </iframe>
                """, unsafe_allow_html=True)
